	...
	if(!empty($_POST)){
		if(!isset($_SESSION['user'])) {
			...
			if(empty($old_password) || empty($password) || empty($confirm_password)) {
				...
				$code = 40100;
				$msg  = '两次输入的密码不一样';
			}else if($_SQL->query("SELECT password FROM user WHERE id = {$user_id}")[0]['password'] != $old_password ) {
				$code = 40100;
				$msg  = '原密码错误，请重新输入';
			}else {
				$password = md5(md5($password).SEC);//加密处理
				if($_SQL->prepare("UPDATE user set password = ? WHERE id = {$user_id}",[$password])) {
					$code = 0;
					$msg  = '修改成功';
					unset($_SESSION['user']);
